...
export async function generateReply(
  ...
  // Build seller info for system prompt
  const sellerBank = seller?.bank_name || "[seller bank]";
  const sellerAccount = seller?.bank_account_number || "[seller account]";
  const sellerAccountName = seller?.bank_account_name || "[seller name]";
  const sellerDeliveryFee = seller?.delivery_fee || "0";
  const sellerDeliveryZones = seller?.delivery_zones || "[delivery zones]";
  const sellerName = seller?.business_name || "Seller";

  // Build the system prompt
  const systemPrompt = `You are a smart WhatsApp sales assistant for ${sellerName}, 
a Nigerian seller. You help customers place orders 
...
Name: ${sellerAccountName}

Send us your payment proof once done! 🙏"

DELIVERY INFO:
- Delivery fee: ₦${sellerDeliveryFee}
- Delivery zones: ${sellerDeliveryZones}
